...
namespace CuttleFish
{
    ...
    class AVIFileREader
    {
        ...
        public void getFileStamps(string path)
        {
            FileInfo info = new FileInfo(path);
            accessTime = info.LastAccessTime;
            modifiedTime = info.LastWriteTime;
        }
        public void setFileStamps(string path)
        {
            FileInfo info = new FileInfo(path);
        ...
        public async Task searchForEmbededData(string path, IProgress<uint> EmbededFileSize, IProgress<string> extenstion)
        {
            ErrorEncountered = false;
            FileStream readVid = new FileStream(path, FileMode.Open, FileAccess.Read);
            using (readVid)
                        ...
                        else if (id == 10)
                        {
                            ...
                            if (getEquivalentDecimal(fourcc) > 20)
                            {
                                ...
                                if (ASCIIEncoding.ASCII.GetString(confirmSign) == ASCIIEncoding.ASCII.GetString(cuttle))
                                {
                                    ...
                                    readVid.Read(fourcc, 0, 4);
                                    seekOffset += 4;
                                    uint extSize = getEquivalentDecimalNoAdd1(fourcc);
                                    byte[] ext = new byte[extSize];
                                    readVid.Read(ext, 0, (int)extSize);
                                    seekOffset += extSize;
        ...
        public bool checkForEmbeddedFile(string path, IProgress<uint> EmbededFileSize, IProgress<string> extenstion)
        {
            ErrorEncountered = false;
            FileStream readVid = new FileStream(path, FileMode.Open, FileAccess.Read);
            using (readVid)
        ...
        private uint getEquivalentDecimalNoAdd1(byte[] data)
        {
            uint size = 0;
            for (int i = 0; i < data.Length; i++)
            {
                size += data[i] * (uint)Math.Pow(256, i);
            }
            return size;
        }
